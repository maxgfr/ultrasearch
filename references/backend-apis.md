...
| `stackexchange` | `GET https://api.stackexchange.com/2.3/search/advanced?site=stackoverflow&filter=withbody` | Anonymous quota is small — one page only. Body is HTML → stripped to text. |
...
| `arxiv` | `GET http://export.arxiv.org/api/query?search_query=all:…` | Atom XML; parsed for title/summary/id/authors/year. |
...
- Rate-limited backends (GitHub, StackExchange, Semantic Scholar) do **one**
  request per run.
- When a backend is throttled or down, it returns no items + a note — the run
  continues on the others, and you should enrich via your own WebSearch.
