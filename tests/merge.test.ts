...
describe("runMerge", () => {
  it("produces stable S# ids: merging the same inputs twice is byte-identical", () => {
    ...
    subDossier(d2, "facet two", [{ url: "https://c.test/z", title: "Z", backend: "wikipedia", score: 1, snippet: "z", text: longText("gamma unique three") }]);
  ...
  it("collapses a URL surfaced by two sub-questions and records both in provenance", () => {
    ...
    const d1 = join(root, "r1");
    const d2 = join(root, "r2");
    const shared: RawSource = {
      url: "https://shared.test/page",
      title: "Shared",
      backend: "duckduckgo",
      score: 1,
      snippet: "s",
      text: longText("shared content here"),
    };
    subDossier(d1, "first sub-question", [
      shared,
      { url: "https://a.test/1", title: "A", backend: "duckduckgo", score: 0.5, snippet: "a", text: longText("alpha distinct apple orchard meadow") },
    ]);
    subDossier(d2, "second sub-question", [
      shared,
      { url: "https://b.test/2", title: "B", backend: "duckduckgo", score: 0.5, snippet: "b", text: longText("beta separate zebra mountain river") },
    ]);
    const m = join(root, "m");
    ...
    expect(sharedSrc.meta.provenance.map((p: any) => p.subQuestion).sort()).toEqual(["first sub-question", "second sub-question"]);
    rmSync(root, { recursive: true, force: true });
  });
    ...
    subDossier(d1, "q1", [
      {
        ...
        score: 1,
        snippet: "p",
        text: longText("paper text alpha"),
        meta: { doi: "10.1000/xyz" },
      },
    ]);
    subDossier(d2, "q2", [
      {
  ...
  it("round-trips extract text through the master dossier", () => {
    ...
    const d1 = join(root, "r1");
    subDossier(d1, "q1", [
      { url: "https://rt.test/p", title: "RT", backend: "duckduckgo", score: 1, snippet: "rt", text: longText("round trip body content alpha") },
    ]);
    const m = join(root, "m");
  ...
  it("stamps the master manifest with the sub-dossiers' ACTUAL depth (a standard fan-out stays standard)", () => {
    ...
    const d1 = join(root, "r1");
    const d2 = join(root, "r2");
    subDossier(d1, "q one", [{ url: "https://a.test/1", title: "A", backend: "duckduckgo", score: 1, snippet: "a", text: longText("a content") }], undefined, "standard");
    subDossier(d2, "q two", [{ url: "https://b.test/1", title: "B", backend: "duckduckgo", score: 1, snippet: "b", text: longText("b content") }], undefined, "standard");
    const m = join(root, "m");
  ...
  it("mixed input depths stamp the deepest; a depth-less (pre-field) input falls back to deep", () => {
    ...
    const d2 = join(root, "r2");
    subDossier(d1, "q one", [{ url: "https://a.test/1", title: "A", backend: "duckduckgo", score: 1, snippet: "a", text: longText("a content") }], undefined, "standard");
    subDossier(d2, "q two", [{ url: "https://b.test/1", title: "B", backend: "duckduckgo", score: 1, snippet: "b", text: longText("b content") }], undefined, "deep");
