...
const TRACKED = [
  // The rendered dossier: check:build re-renders it from the committed run, so
  // a webindex change to the renderer moves these bytes. The `orchestrate`
  // snapshot is the RUNBOOK this skill writes itself — deliberately NOT listed,
  // so it stays a hard red.
