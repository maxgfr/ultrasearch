...
export function mdToHtml(md: string, idPrefix: string, opts: { verdicts?: Map<string, VerdictKind> } = {}): { html: string; headings: Heading[] } {
...
function verificationSection(r: VerifyResult): string {
  ...
  const table = rows
    ...
    : "";
  const srcLinks = (ids: string[]): string => ids.map((s) => `<a href="#src-${escapeHtml(s)}">[${escapeHtml(s)}]</a>`).join(" ");
...
// A compact verification block for the consolidated markdown report.
function verificationMarkdown(r: VerifyResult): string {
  const status = r.ok ? "**grounded**" : `**${r.failures.length} claim(s) failed**`;
  const counts = `supported ${r.supported} · partial ${r.partial} · refuted ${r.refuted} · unsupported ${r.unsupported}`;
  ...
  if (contras.length) {
    ...
    for (const c of contras) {
      out.push(
        `- **${c.claimId}**: supported by ${c.supporting.map((s) => `[${s}]`).join(" ")} · ` +
          `refuted by ${c.refuting.map((s) => `[${s}]`).join(" ")}${c.note ? ` — ${c.note}` : ""}`,
      );
    }
    out.push("");
  }
...
export function buildReportMarkdown(dir: string): string {
  ...
  const meta =
    ...
    `${manifest.lang ? ` · lang ${manifest.lang}` : ""}${manifest.region ? `/${manifest.region}` : ""}` +
    ` · ${manifest.builtAt} · generated by ultrasearch`;
